...
class DashboardService:
    ...
    def get_summary(db: Session, current_user: User):
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail="Database error while fetching summary"
            )
    ...
    def get_category_breakdown(db: Session, current_user: User):
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="Database error while fetching category breakdown"
            )
    ...
    def get_recent_activity(db: Session, current_user: User, limit: int = 5):
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="Database error while fetching recent activity"
            )
    ...
    def get_monthly_trends(db: Session, current_user: User, year: int = None):
        ...
        for month in range(1, 13):
            ...
            try:
                ...
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail=f"Database error while fetching monthly trends for month {month}"
                )
