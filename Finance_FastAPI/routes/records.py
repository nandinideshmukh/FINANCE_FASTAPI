...
@router.get("/")
async def view_records(page: int = 0, limit: int = 100, db: Session = Depends(get_db), current_user: User = Depends(get_current_user),search:str = None):
    return  RecordService.view_records(db, current_user, page, limit,search)
